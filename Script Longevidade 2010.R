...
plot(semilong2010)#gráfico com a distribuição dos pontos na malha
summary(semilong2010)
variolong2010<-variog(semilong2010, max.dist=2.5)#máxima distância para cálculo das semivariâncias deve estar entre 50% e 75% da distância máxima 
variolong2010#mostra informações como distâncias($u), semivariâncias($v), número de pares($n) e desvio padrão($sd)
plot(variolong2010,main="semivariograma médio - Longevidade 2010")
...
summary(Regm1) 
plot(valm1$data,valm1$predicted, main="Validação cruzada - Longevidade 2010")
...
title(main="Mapa da variância (Longevidade 2010)")
